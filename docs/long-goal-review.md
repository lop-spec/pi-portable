...
- 每轮先读本机清单，剔除 Markdown/HTML 注释 `<!-- ... -->` 后再检查，支持单行、多行和行内注释；未闭合的 `<!--` 从起点到文件末尾均视为注释。注释中的数字和“目标”不参与判断；全是注释（包括只保留标准清单标题）、空文件、仅清单标题、明确的空清单，或生效文本既没有数字也没有“目标”两个字时，直接记录原因并跳过，不查会话、不调用模型、不发送消息。混合清单仅将未注释内容交给模型，决策校验和投递也拒绝注释中的目标；原文件及注释不改写，发送前仍按原始文件哈希检查并发修改。Markdown 的 `#` 标题不是注释。有数字或“目标”任一项即可通过检查；数字包括半角、全角等十进制数字。通过后仍由模型理解自然语言，不需标识、字段或固定标题。
...
- **同一长目标累计已送达提醒超过3次（至少4次）仍未达标时，先反思巡检建议本身，优化后再继续提醒。** 核对此前建议的依据、执行反馈及无效/重复环节；reason写明反思与改进，advice给优化后的下一步和验证。没有有依据的改进就observe，不原样或换措辞催促。session(section=recent)提供会话级reviewAdviceCount，跨巡检模型、压缩累计，不受最近3条窗口限制；多目标会话用users核对同目标实际提醒，不计observe或未送达尝试。复用本轮评审，不新增模型调用、账本或强停原任务；此规则由巡检模型结合历史执行，不将文本合同测试声称为反思质量保证。
