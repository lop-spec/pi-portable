...
7. **模型执行与 S6 对抗预审**
   ...
   - 后台对抗预审检查 scope/cheaper/evidence。S5 已取得当前证据时，审查任务被确认并主动取消，避免无价值的第二次模型调用和悬挂连接。
8. **S7 工具门**
...
最终双循环报告：`data/validation/20260830-030206-pi-five-chain-recursion-fix/report.json`。第二轮所有受门控阶段均严格低于历史最佳的 50%：

| 排名/类型 | 初始模型：历史 → 50%门 → 当前 | 后续模型：历史 → 50%门 → 当前 |
|---|---:|---:|
| 1 排查修复 | 12086 → `<6043` → **22.9 ms** | 6712 → `<3356` → **0 ms** |
| 2 实现改动 | 33720 → `<16860` → **23.7 ms** | 7240 → `<3620` → **0 ms** |
| 3 解释建议 | 4397 → `<2198.5` → **22.2 ms** | 历史为 0，不设性能门 |
| 4 查询审计 | 7351 → `<3675.5` → **25.1 ms** | 4994 → `<2497` → **0 ms** |
| 5 运行运维 | 13075 → `<6537.5` → **23.1 ms** | 3106 → `<1553` → **0 ms** |

五类两轮均通过答案、工具/读回、S2 3×、S3 使用、S4 集合、S8 derived canonical 和工作区窄范围门。热 Pi 历史扫描由全量迁移的 `3210.8 ms` 降为线性追加 `94.7 ms`，无变化复扫为 `6.3 ms`。反事实报告 `data/validation/20260830-030235-pi-hard-gate-probes-recursion-fix/report.json` 三门全绿。

历史引用回答不会再递归成为事实源：`SSH 双向免密是否成功？请只根据相关历史结论回答。` 即使已有一条错误的 exact 回答，仍跳过该 wrapper 并命中原始成功事件 `e_live_d7538c9cc3e831c732f200df`，`relevance=1`；错误事件不删除，继续保留审计。真实模型报告位于 `data/validation/20260830-030145-live-v792-recursion-fix/`，答案引用两个方向的实测事实、唯一 `history-used`、S4 actual=oracle、S8 canonical 均通过；SSH/TLS 协议解释反例继续 miss。
