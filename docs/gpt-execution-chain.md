...
最终双循环报告：`data/validation/20260829-202347-pi-five-chain-final-v7-v792/report.json`。第二轮所有受门控阶段均严格低于历史最佳的 50%：

| 排名/类型 | 初始模型：历史 → 50%门 → 当前 | 后续模型：历史 → 50%门 → 当前 |
|---|---:|---:|
| 1 排查修复 | 12086 → `<6043` → **20.3 ms** | 6712 → `<3356` → **0 ms** |
| 2 实现改动 | 33720 → `<16860` → **20.8 ms** | 7240 → `<3620` → **0 ms** |
| 3 解释建议 | 4397 → `<2198.5` → **23.4 ms** | 历史为 0，不设性能门 |
| 4 查询审计 | 7351 → `<3675.5` → **20.0 ms** | 4994 → `<2497` → **0 ms** |
| 5 运行运维 | 13075 → `<6537.5` → **19.5 ms** | 3106 → `<1553` → **0 ms** |

五类两轮均通过答案、工具/读回、S2 3×、S3 使用、S4 集合、S8 derived canonical 和工作区窄范围门。热 Pi 历史扫描由全量迁移的 `3210.8 ms` 降为线性追加 `94.7 ms`，无变化复扫为 `6.3 ms`。反事实报告 `data/validation/20260829-202447-pi-hard-gate-probes-final-v792/report.json` 三门全绿。
