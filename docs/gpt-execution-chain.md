...
7. **模型执行、S6 对抗预审与两态目标循环**
   ...
   - persistent checklist 复用 `goal-redirector` 的失败指纹：相同未完成结果第二轮进入 evidence、新证据轮，第三轮进入 tabu、换模块/层次/根因；真实进展信号变化会重置该换向序列。所有 continuation 都先检查过期 `nextAction`，但只接受同一逻辑记录行首的精确动作键；`staleNextAction` 指标、源码片段中间及后续日志时间戳不得拼成伪动作。时间受阻时必须生成至少两个独立方向并执行最佳方向，禁止以前台 sleep/轮询冒充持续执行。
