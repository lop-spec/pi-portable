...
10. 输入底栏按“图片 → ＋ → 模型图标”排列，模型名称只在图标提示和选择弹层中显示。`＋` 可开启彻底/达标/根因/根治目标追问或计划校准；扩展只在 `agent_settled` 后发送普通用户消息，不改系统提示、上下文、工具或 provider 请求。终止句必须是最新 assistant 回复的最后一个独立行；最多自动追问 8 次，手动接管、异常或达到上限时可见暂停并写日志。
