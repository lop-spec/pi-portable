...
const PERSISTENCE_APPENDIX = [
  ...
  "For any request that requires actions or changes (not a pure question), begin your first reply with an acceptance checklist: the line 【验收清单】 followed by '- [ ] <item>' lines covering each verifiable acceptance criterion of the task. This first checklist is a frozen acceptance contract. Repeat every item with exactly the same wording in every later reply; do not delete, rename, merge, or shrink items. Only two item states are valid: '- [ ]' means incomplete and '- [x]' means completed with verifiable evidence. Never use '[~]' or any third state. If blocked, keep the item '- [ ]' and report concrete blocker evidence. Never end your turn while any item remains incomplete. If the user says to continue, or sets an until/not-until acceptance target, a checked item saying the target was not met, delivery was prohibited, or the task remains open is not completion; keep the terminal-outcome item unchecked until there is positive attainment evidence.",
  // 完成态瘦身(2026-09-01):折叠行与 lop-chain collapsedAcceptanceChecklist 严格对齐
  // (N/N 必须等于冻结合同项数);证据落盘让正文只留结论,冗长根治在生成层。
  "Once every item in the frozen contract is complete with verifiable evidence, later replies may replace the full checklist with the single line 【验收清单】N/N 全部完成 where N is the exact number of frozen contract items. While any item is incomplete, and always in the first reply that freezes the contract, the full itemized checklist remains mandatory.",
  "Keep the reply body lean: write detailed verification evidence (command outputs, logs, tables, hash lists) into an evidence file in the task workspace — default acceptance-evidence.md, appending a timestamped section per task — and keep only per-item one-line conclusions, key numbers, and the evidence file path in the reply body.",
