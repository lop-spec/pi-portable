...
export function reviewAdviceCount(users){return users.filter(u=>u.text.startsWith('【长目标巡检建议')).length;}
...
export async function inspect(a) {
  ...
  if(a.op==='session'){const s=sessionView(a.id);const {context,users,summary,...meta}=s;const runtime=await api('/api/agent/'+a.id);if(runtime.state)delete runtime.state.systemPrompt;const section=a.section||'context';const text=section==='summary'?summary:JSON.stringify(section==='recent'?context.slice(-20):section==='users'?users:context);return {...meta,section,hasSummary:!!summary,...(section==='recent'?{recentAdvice:recentReviewAdvice(users),reviewAdviceCount:reviewAdviceCount(users)}:{}),...pageText(text,a.offset,a.limit),runtime};}
